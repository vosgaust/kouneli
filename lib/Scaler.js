...
class Scaler {
  ...
  monitor() {
    this.interval = setInterval(() => {
      this.currentPromise = this.currentPromise.then(() => this._checkQueueStatus(this.queue));
      return this.currentPromise;
    }, this.interval * 1000);
  }

  _checkQueueStatus() {
    return this.rabbitManager.getMessagesInQueue(this.queue)
    ...
    .then((result) => {
      ...
      } else if(response.unacked === 0 && currentReplicas > this.minCount && !response.ready) {
